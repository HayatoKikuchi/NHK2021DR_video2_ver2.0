...
coords ManualControl::getLocalVel(coords globalVel, double robotAngle)
{
  coords refVel;
  refVel.x = +globalVel.x*cos(robotAngle) + globalVel.y*sin(robotAngle);
  refVel.y = -globalVel.x*sin(robotAngle) + globalVel.y*cos(robotAngle);
  refVel.z = globalVel.z;
