...
#define JOY_MAXANGVEL   ( radians(180.0) )
#define MANUAL_LOWPASS_T  ( 0.1 )
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

// >>> PathTrackingで使用 >>>>>>>>>>>>>>>>>>
#define POSI_X_KP    ( 1.2 )
#define POSI_X_KI    ( 0.0 )
#define POSI_X_KD    ( 1.0 )

#define POSI_Y_KP    ( 1.2 )
#define POSI_Y_KI    ( 0.0 )
#define POSI_Y_KD    ( 1.0 )

#define POSI_Z_KP    ( 6.0 )
#define POSI_Z_KI    ( 0.0 )
#define POSI_Z_KD    ( 0.5 )
...
#define VEL_Z_KP    ( 0.75 )
#define VEL_Z_KI    ( 0.0 )
#define VEL_Z_KD    ( 0.75 )
...
#define DRIVE_UNIT  ( PLATFORM_MECHANUM )
    ...
    #define TREAD_2 	( 0.593/2.0 ) //車両中心から車輪接地点までのY軸方向距離(トレッド/2)
    #define WHEELBASE_2	( 0.430/2.0 ) //車両中心から車輪接地点までのX軸方向距離(ホイールベース/2)
    #define _2RES_PI  ( 2.0 * 500 / PI ) //  駆動輪の角速度[rad/s]からRoboClawの指令値[pulses/s]に変換するための定数(500はエンコーダ分解能) 4逓倍しているが，分母は元は2*piで，通分されている
...
#define CON_TYPE  ( CON_DS4 )
    ...
    #define BUTTON_PS     9 // PS4のときはPSボタン
    #define BUTTON_PAD    10 // PS4のときはパッド
